angular.module(GOLFPRO).provider('loginStatusProvider', [function() {
	...
	var validateAuthenticationPromise = function() {
		//Check to make sure one of the providers has been logged in
		var getFacebookAuthenticationPromise = waitForInit().then(function() {
			return new Promise(function(s, f) { facebookConnectPlugin.getLoginStatus(s, f); }).then(function(authentication) {
				if(authentication.status !== 'connected') { return null; }
				var normalizedAuth = { Login: {}, Type: 'Facebook', UserId: authentication.authResponse.userID };
				normalizedAuth.Login['graph.facebook.com'] = authentication.authResponse.accessToken;
				return normalizedAuth;
			}, function(failure) { console.error(failure); return null; });
		});
		var getGoogleAuthenticationPromise = Promise.resolve().then(function() {
			return new Promise(function(s, f) { window.plugins.googleplus.trySilentLogin(GOOGLEPLUS_AUTH_INFO, s, f); }).then(function(authentication) {
				var normalizedAuth = { Login: {}, Type: 'Google', UserId: authentication.userId };
				normalizedAuth.Login['accounts.google.com'] = authentication.idToken;
				return normalizedAuth;
			}, function(failure) { /* Failure is normal mode for google auth, just means you need to login directly. */ return null; });
		});
		...
		return Promise.all([getFacebookAuthenticationPromise, getGoogleAuthenticationPromise, getCognitoUserPoolAuthenticationPromise])
	...
	var service = {
		...
		attemptFacebookLoginPromise: attemptFacebookLoginPromise,
		attemptGoogleLoginPromise: attemptGoogleLoginPromise,
